...
// db.users.insertMany([{name:"raj"},{name:"kumar"}]);
...
db.user.find({name:{$eq: "ravi"}})
// db.users.find({ name: { $eq: "tanmay" } });
db.user.find({name:{$ne:"ravi"}})
...
db.user.find({$and:[{name:"ravi"},{age:20}]})
// db.users.find({ $or: [{ name: "tanmay" }, { age: 19 }] });
db.users.find({$or:[{name:"ravi"},{age:20}]})
